...
export async function GET(req:NextRequest){try{await authenticate(req);const u=new URL(req.url);const page=Math.max(1,Number(u.searchParams.get('page')??1));const limit=Math.min(100,Math.max(1,Number(u.searchParams.get('limit')??20)));const [data,total]=await Promise.all([prisma.article.findMany({skip:(page-1)*limit,take:limit,orderBy:{createdAt:'desc'},include:{scholarship:true,seo:true,qualityChecks:true,publications:true}}),prisma.article.count()]);return apiResponse({data,total,page,limit,totalPages:Math.ceil(total/limit)})}catch(e){return handleApiError(e)}}
